...
def get_models():
    return jsonify(_paginate([model.to_primitive(role='DTO') for model in data.get_models()]))
...
def get_model(model_name):
    ...
    return jsonify(model.to_primitive(role='DTO')) if model else abort(404)


@rest_api.route('/models/<model_name>', methods=['POST'])
def run_model(model_name):
    if data.model_exists(model_name):
        run_task(model=data.get_model(model_name).to_primitive(), arguments=request.data if request.data else '')
